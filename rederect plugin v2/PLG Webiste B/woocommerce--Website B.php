...
Version: 13.5.2 // Incremented version to reflect changes
Author: CODARAB PAY
...
function wc_prefilled_checkout_fields()
{
    ...
    // Set billing fields with validation
    ...
    if (isset($_GET['billing_email'])) {
        ...
        if (is_email($billing_email)) {
            ...
            error_log("Invalid billing email provided: " . $_GET['billing_email']);
            WC()->customer->set_billing_email(''); // Set to empty string to avoid fatal error
        }
    }
    if (isset($_GET['billing_company'])) {
        WC()->customer->set_billing_company(sanitize_text_field($_GET['billing_company']));
    }
    if (isset($_GET['billing_address_1'])) {
        WC()->customer->set_billing_address_1(sanitize_text_field($_GET['billing_address_1']));
    }
    if (isset($_GET['billing_address_2'])) {
        WC()->customer->set_billing_address_2(sanitize_text_field($_GET['billing_address_2']));
    }
    if (isset($_GET['billing_city'])) {
        WC()->customer->set_billing_city(sanitize_text_field($_GET['billing_city']));
    }
    if (isset($_GET['billing_state'])) {
        WC()->customer->set_billing_state(sanitize_text_field($_GET['billing_state']));
    }
    if (isset($_GET['billing_postcode'])) {
        WC()->customer->set_billing_postcode(sanitize_text_field($_GET['billing_postcode']));
    }
    if (isset($_GET['billing_country'])) {
        WC()->customer->set_billing_country(sanitize_text_field($_GET['billing_country']));
    }
    ...
    if (isset($_GET['currency'])) {
        global $woocommerce;
        $woocommerce->session->set('order_currency', sanitize_text_field($_GET['currency']));
    }
...
function update_external_order_id_and_url_meta($order_id, $posted_data, $order)
{
    ...
    if (isset($_COOKIE['secondsite'])) {
        update_post_meta($order_id, 'hook_url', sanitize_text_field($_COOKIE['secondsite']));
    }
}
...
function send_scroll_height_to_parent()
{
    ...
    <script type="text/javascript">
        ...
        function sendScrollHeight() {
            ...
            if (scrollHeight === prevHeight) return;
            if (repeat > 3) return;
            repeat++;
